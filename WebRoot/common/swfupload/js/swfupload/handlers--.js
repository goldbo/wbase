...
function uploadProgress(file, bytesLoaded) {
	...
	try {
		...
		var progress = new FileProgress(file, file.id);
...
function addFileInfo(fileId,message){
	...
	var txt =  "<font color='green'>"+message+"</font>";
	document.getElementById("divFileProgress"+fileId).innerHTML = txt;
}
...
function addReadyFileInfo(file){
	...
	col3.innerHTML = file.size+"KB";
	col4.innerHTML = "<div id = 'divFileProgress"+file.id+"'>"+file.status1+"</div>";
	col5.innerHTML = "<a href='javascript:deleteFile(\""+file.id+"\")'>取消</a>";
}
...
function cancelUpload(){
	...
	for(var i=1,j=0;i<rows.length;j++,i++){
		ids[j] = rows[i].id;
	}	
...
function uploadComplete(file) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			var infoTable = document.getElementById("infoTable");
        	var length = infoTable.rows.length;
	        //alert(length);
	        // 倒序遍历，这样才能顺利删除
...
function uploadError(file, errorCode, message) {
	...
	try {
		...
		case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
			try {
				progress = new FileProgress(file,  file.id);
...
function FileProgress(file, targetID) {
	this.fileProgressID = "divFileProgress2"+targetID;
	
	var progressDiv = "divFileProgress"+targetID;
	...
	if (!this.fileProgressWrapper) {
		...
		//document.getElementById("divFileProgress"+targetID).style.height = "75px";
		document.getElementById("divFileProgress"+targetID).appendChild(this.fileProgressWrapper);
