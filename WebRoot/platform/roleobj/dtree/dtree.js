...
//显示已选择的节点dTree.prototype.showNode = function(nodePid) {	document.getElementById("c" + this.obj + nodePid).checked=true;		};
...
};//点击复选框onclick事件//复选框onclick事件dTree.prototype.cc=function(nodeId, nodePid){        //首先获取这个多选框的id     var cs = document.getElementById("c" + this.obj + nodeId).checked;    var n,node = this.aNodes[nodeId];        var len = this.aNodes.length;    if(!this.config.isSingle){   //多选        /*for (n=0; n<len; n++) { //如果循环每一个节点         if (this.aNodes[n].pid == nodeId) { //如果选中的是非叶子节点,则要将所有的子节点选择和父节点一样             document.getElementById("c" + this.obj + this.aNodes[n].id).checked = cs;                       this.cc(this.aNodes[n].id, nodeId);                   }             }           if(cs==true){  //当前是选中状态        var pid=nodePid;          var bSearch;          do{              bSearch=false;              for(n=0;n<len;n++){  //循环每一个节点                if(this.aNodes[n].id==pid){  //如果循环的节点的id等于PID                     document.getElementById("c"+this.obj+pid).checked=true; //那么这个循环的节点应该被选中                      pid=this.aNodes[n].pid;                      bSearch= true;                            break;                    }                            }            }while(bSearch==true);      }  	if(cs==true){ 		var pid = nodePid;          do{                for(j=0;j<len;j++){         //循环每一个多选框  如果这个节点的子节点有其他是选中的,则不取消                 if(this.aNodes[j].pid==pid && document.getElementById("c" + this.obj + this.aNodes[j].id).checked==false){                                     return;                  }              }              if(j==len){   //循环结束                 for(k=0;k<len;k++){                        if(this.aNodes[k].id==pid){   //如果找到父节点                         document.getElementById("c"+this.obj+this.aNodes[k].id).checked=true;                            pid=this.aNodes[k].pid;                           break;                      }                       }                }            }while(pid!=-1); 	} 	      if(cs==false){      //如果被取消选择         var pid = nodePid;          do{                for(j=0;j<len;j++){         //循环每一个多选框  如果这个节点的子节点有其他是选中的,则不取消                 if(this.aNodes[j].pid==pid && document.getElementById("c" + this.obj + this.aNodes[j].id).checked==true){                                     document.getElementById("c"+this.obj+this.aNodes[j].pid).checked=false;                    return;                  }              }              if(j==len){   //循环结束                 for(k=0;k<len;k++){                        if(this.aNodes[k].id==pid){   //如果找到父节点                         document.getElementById("c"+this.obj+this.aNodes[k].id).checked=false;                            pid=this.aNodes[k].pid;                           break;                      }                       }                }            }while(pid!=-1);          } */    }else{  //单选    	for(var aa=0;aa<len;aa++){    		if(this.aNodes[aa]!=nodeId){    			document.getElementById("c" + this.obj + this.aNodes[aa].id).checked = false;    		}      	}    	document.getElementById("c" + this.obj + nodeId).checked = cs;    }}//获取复选框 节点ID 集合dTree.prototype.getCheckBoxValue=function(){    var chkIds = "";    var len = this.aNodes.length;    var k = 0    for (var i = 0; i < len; i++){    	var nid = this.aNodes[i].id;    	if(nid!=0){	    	var ched = document.getElementById("c" + this.obj + nid ).checked;	        if (ched==true){	        	if(k==0){	        		chkIds+=nid;	        	}else{	            	chkIds +=","+nid;	        	}	            k++;	        }    	}    }    return chkIds;}//获取复选框 节点ID 及 值 集合dTree.prototype.getCheckTreeIDValue=function(){    var chkIdNames = new Array();    var len = this.aNodes.length;    for (var i = 0; i < len; i++){    	var nid = this.aNodes[i].id;    	var nName = this.aNodes[i].name;    	if(this.config.isSingle){	    	var ched = document.getElementById("c" + this.obj + nid ).checked;	        if (ched==true){	        	chkIdNames.push(nid+","+nName);	        }    	}else{    		if(nid!=0){	    	var ched = document.getElementById("c" + this.obj + nid ).checked;	        if (ched==true){	        	chkIdNames.push(nid+","+nName);	        }    	}    	}    }    return chkIdNames;}//获取复选框 节点ID 及 值 集合dTree.prototype.getCheckBoxIDValue=function(){    var chkIdNames = new Array();    var len = this.aNodes.length;    for (var i = 0; i < len; i++){    	var nid = this.aNodes[i].id;    	var nName = this.aNodes[i].name;    	if(this.config.isSingle){	    	var ched = document.getElementById("c" + this.obj + nid ).checked;	        if (ched==true){	        	chkIdNames.push(nid);	        	chkIdNames.push(nName);	        }    	}else{    		if(nid!=0){	    	var ched = document.getElementById("c" + this.obj + nid ).checked;	        if (ched==true){	        	chkIdNames.push(nid);	        	chkIdNames.push(nName);	        }    	}    	}    }    return chkIdNames;}//获取所有复选框 节点ID 及 值 集合dTree.prototype.getAllCheckTreeIDValue=function(){    var chkIdNames = new Array();    var len = this.aNodes.length;    for (var i = 0; i < len; i++){    	var nid = this.aNodes[i].id;    	var nName = this.aNodes[i].name;    		if(nid!=0){	        	chkIdNames.push(nid+","+nName);	        }    }    return chkIdNames;}